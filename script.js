...
(function() {
  ...
  function saveProjects(doPublish) {
    ...
    updateStats();
    updateFilterBtns();
    /* Auto-publish when admin saves (silent, no callback) */
  ...
  function loadProjects() {
    ...
    /* Merge published projects from GitHub */
    fetchPublishedProjects(function(published) {
      ...
      for (var i = 0; i < published.length; i++) {
        if (!ids[published[i].id]) {
          projects.push(published[i]);
        }
      }

      /* Merge auto-imported apps from apps-linux-only */
      fetchAppsRepo(function(apps) {
        ...
        renderAll();
        secLog('Merged ' + published.length + ' published + ' + apps.length + ' auto-imported projects', 'ok');
      });
    });
  }
  ...
  function fetchPublishedProjects(callback) {
    ...
    xhr.onload = function() {
      ...
      /* file not found or invalid — use empty */
      callback([]);
    };
  ...
  function publishToSite(callback) {
    ...
    xhrGet.onload = function() {
      ...
      /* 404 means file doesn't exist yet — that's fine, sha stays empty */
      ...
      xhrPut.onload = function() {
        if (xhrPut.status === 200 || xhrPut.status === 201) {
          ...
          /* token might be bad — clear it so user can re-enter */
