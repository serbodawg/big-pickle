...
(function() {
  ...
  var PWD_HASH_KEY = 'bp_pwd_hash';
  var PWD_SALT_KEY = 'bp_pwd_salt';
  ...
  function setSession() {
    var token = 'bp_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10);
  ...
  function openLoginModal() {
    var isFirst = !isPasswordSet();
    if (isFirst) {
      if (loginTitle) loginTitle.textContent = t('login_setup_title');
      if (loginLabel) loginLabel.textContent = t('login_setup_label');
      if (loginSubmit) loginSubmit.textContent = t('login_setup_btn');
      if (loginHint) loginHint.removeAttribute('hidden');
    } else {
  ...
  function verifyPassword(password) {
    var storedHash = localStorage.getItem(PWD_HASH_KEY);
    var salt = localStorage.getItem(PWD_SALT_KEY);
    if (!storedHash || !salt) {
  ...
  function setAuthenticated(val) {
    ...
    for (var i = 0; i < adminEls.length; i++) {
      if (authenticated) {
        adminEls[i].removeAttribute('hidden');
      } else {
        adminEls[i].setAttribute('hidden', '');
      }
    }
  }
  ...
  function handleLoginSubmit(e) {
    ...
    if (isPasswordSet()) {
      ...
      if (pwd.length < 4) {
        showLoginError(t('login_error_short'));
        return;
      }
      setupPassword(pwd);
    }
  ...
  function checkSession() {
    if (isPasswordSet() && isSessionValid()) {
      setAuthenticated(true);
    }
  }
